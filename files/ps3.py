    ...
    while calculate_handlen(hand) > 0:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(word, hand, word_list) == True:
                ...
                print(f'"{word}" earned {get_word_score(word, calculate_handlen(hand))} points. Total: {total_score} points\n')
...
def substitute_hand(hand, letter):
    ...
    new_hand = hand.copy()
    new_letter = random.choice([i for i in string.ascii_lowercase if i not in new_hand.keys() and i != letter])
    try:
        new_hand.update({new_letter: new_hand.pop(letter.lower())})
        return new_hand
    except KeyError:
        return new_hand
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    substitue_option_used = False

    replay_option_used = False

    while total_number_of_hands > 0:
        hand = deal_hand(HAND_SIZE)

        if substitue_option_used == False:
